...
namespace PordznakanAPI.Controllers
{
    ...
    [ApiController]
    public class LogMmuhStudentController : ControllerBase
    {
        private readonly AppDbContext _context;
        private readonly ILogger<LogMmuhStudentController>? _logger;
        private readonly IConfiguration _configuration;
        private readonly HttpClient _httpClient;

        public LogMmuhStudentController(
            AppDbContext context,
            IConfiguration configuration,
            ILogger<LogMmuhStudentController>? logger = null)
        {
            _context = context;
        ...
        private JArray ParsePhpArrayFormat(string phpOutput)
        {
            ...
            for (int i = 0; i < objectMatches.Count; i++)
            {
                ...
                // Extract school_id - pattern: [school_id] => 930
                ...
                // Extract action_date - pattern: [action_date] => 2026-02-13 10:11:50
                ...
                // Extract method - pattern: [method] => StudentEdit
        ...
        public async Task ProcessAllRegions()
        {
            ...
            foreach (var regionId in regionIds)
            {
                try
                {
                    _logger?.LogInformation($"Processing LogMmuhStudent logs for region {regionId}...");
                    
                    // Read data from source API
                    using var sourceClient = new HttpClient();
                    var sourceUrl = $"https://api.emis.am/v1/dshh_log_mmuh_students/{regionId}";
                    ...
                    foreach (var logToken in logArray)
                    {
                        ...
                        try
                        {
                            ...
                            var existingLog = await _context.LogMmuhStudents
                            ...
                            else
                            {
                                ...
                                _context.LogMmuhStudents.Add(newLog);
                                addedCount++;
                            }
                        }
                ...
                {
                    _logger?.LogError(ex, $"Exception while processing LogMmuhStudent logs for region {regionId}");
                }
            }

            _logger?.LogInformation("LogMmuhStudent processing completed for all regions.");
        }

        /// <summary>
        /// Reads log mmuh student data from the API and saves it to the database
        /// Transfer logic is commented out for testing
        /// </summary>
        [HttpPost("transfer/{regionId}")]
        public async Task<IActionResult> TransferLogMmuhStudents([FromRoute] int regionId)
        {
            try
            {
                // Get transfer API URL from configuration (commented out - not used for now)
                // TODO: Add this to appsettings.json: "TransferApi:LogMmuhStudentUrl": "https://your-api-endpoint.com/api/log-mmuh-student"
                // var transferApiUrl = _configuration?["TransferApi:LogMmuhStudentUrl"] 
                //     ?? "https://your-api-endpoint.com/api/log-mmuh-student"; // Placeholder - replace with actual API URL
            ...
            {
                _logger?.LogError(ex, $"Error processing LogMmuhStudent for region {regionId}");
                return StatusCode(500, new
                {
        ...
        /// <summary>
        /// Reads log mmuh student data from the API and saves all regions
        /// </summary>
        ...
        public async Task<IActionResult> TransferAllRegions()
        {
            ...
            foreach (var regionId in regionIds)
            {
                try
                {
                    ...
                    var result = await TransferLogMmuhStudents(regionId);
        ...
        /// <summary>
        /// Gets all log mmuh students from database
        /// </summary>
        [HttpGet]
        public async Task<IActionResult> GetLogMmuhStudents(
            ...
            [FromQuery] int take = 100)
        {
            try
            {
                var query = _context.LogMmuhStudents.AsQueryable();
            ...
            {
                _logger?.LogError(ex, "Error getting log mmuh students");
                return StatusCode(500, new
                {
                    error = "Failed to get log mmuh students",
                    message = ex.Message
                });
            }
        }
        ...
        public async Task<IActionResult> TestDatabase()
        {
            try
            {
                var count = await _context.LogMmuhStudents.CountAsync();
                var sample = await _context.LogMmuhStudents
        ...
        /// <summary>
        /// Gets a specific log mmuh student by ID
        /// </summary>
        [HttpGet("{id}")]
        public async Task<IActionResult> GetLogMmuhStudent(Guid id)
        {
            try
            {
                var log = await _context.LogMmuhStudents.FindAsync(id);

                if (log == null)
                {
                    return NotFound(new { message = "Log mmuh student not found" });
                }
            ...
            {
                _logger?.LogError(ex, $"Error getting log mmuh student {id}");
                return StatusCode(500, new
                {
                    error = "Failed to get log mmuh student",
                    message = ex.Message
                });
            }
        }
