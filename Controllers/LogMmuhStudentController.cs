...
namespace PordznakanAPI.Controllers
{
    ...
    public class LogMmuhStudentController : ControllerBase
    {
        ...
        private const string SourceBaseUrl = "https://api.emis.am/v1/dshh_log_mmuh_students";

        public LogMmuhStudentController(ILogTransferService logTransferService)
        {
            _logTransferService = logTransferService;
        }

        private static LogMmuhStudent MapToModel(JObject o, DateTime fallbackDate)
        {
        ...
        public Task ProcessAllRegions(DateOnly date) =>
            _logTransferService.ProcessAllRegionsAsync(SourceBaseUrl, nameof(LogMmuhStudent), date, MapToModel);
