...
namespace PordznakanAPI.Controllers
{
    ...
    public class LogMmuhEmployeeController : ControllerBase
    {
        ...
        private readonly ILogger<LogMmuhEmployeeController>? _logger;
        private readonly IConfiguration _configuration;
        private readonly HttpClient _httpClient;

        public LogMmuhEmployeeController(
            AppDbContext context,
            IConfiguration configuration,
            ILogger<LogMmuhEmployeeController>? logger = null)
        {
            _context = context;
        ...
        public async Task ProcessAllRegions()
        {
            ...
            foreach (var regionId in regionIds)
            {
                try
                {
                    _logger?.LogInformation($"Processing LogMmuhEmployee logs for region {regionId}...");
                    
                    // Read data from source API
                    using var sourceClient = new HttpClient();
                    var sourceUrl = $"https://api.emis.am/v1/dshh_log_mmuh_emploee/{regionId}";
                    ...
                    foreach (var logToken in logArray)
                    {
                        ...
                        try
                        {
                            ...
                            var existingLog = await _context.LogMmuhEmployees
                            ...
                            {
                                // Add new log
                                var newLog = new LogMmuhEmployee
                                {
                                    Id = Guid.NewGuid(),
                                    LogId = logId,
                                    SchoolId = schoolId,
                                ...
                                _context.LogMmuhEmployees.Add(newLog);
                                addedCount++;
                            }
                        }
                ...
                {
                    _logger?.LogError(ex, $"Exception while processing LogMmuhEmployee logs for region {regionId}");
                }
            }

            _logger?.LogInformation("LogMmuhEmployee processing completed for all regions.");
        }

        /// <summary>
        /// Reads log mmuh employee data from the API and saves it to the database
        /// Transfer logic is commented out for testing
        /// </summary>
        [HttpPost("transfer/{regionId}")]
        public async Task<IActionResult> TransferLogMmuhEmployees([FromRoute] int regionId)
        {
            try
            {
                // Get transfer API URL from configuration (commented out - not used for now)
                // TODO: Add this to appsettings.json: "TransferApi:LogMmuhEmployeeUrl": "https://your-api-endpoint.com/api/log-mmuh-employee"
                // var transferApiUrl = _configuration?["TransferApi:LogMmuhEmployeeUrl"] 
                //     ?? "https://your-api-endpoint.com/api/log-mmuh-employee"; // Placeholder - replace with actual API URL
            ...
            {
                _logger?.LogError(ex, $"Error processing LogMmuhEmployee for region {regionId}");
                return StatusCode(500, new
                {
        ...
        /// <summary>
        /// Reads log mmuh employee data from the API and saves all regions
        /// </summary>
        ...
        public async Task<IActionResult> TransferAllRegions()
        {
            ...
            foreach (var regionId in regionIds)
            {
                try
                {
                    ...
                    var result = await TransferLogMmuhEmployees(regionId);
        ...
        /// <summary>
        /// Gets all log mmuh employees from database
        /// </summary>
        [HttpGet]
        public async Task<IActionResult> GetLogMmuhEmployees(
            ...
            [FromQuery] int take = 100)
        {
            try
            {
                var query = _context.LogMmuhEmployees.AsQueryable();
            ...
            {
                _logger?.LogError(ex, "Error getting log mmuh employees");
                return StatusCode(500, new
                {
                    error = "Failed to get log mmuh employees",
                    message = ex.Message
                });
            }
        }
        ...
        public async Task<IActionResult> TestDatabase()
        {
            try
            {
                var count = await _context.LogMmuhEmployees.CountAsync();
                var sample = await _context.LogMmuhEmployees
        ...
        /// <summary>
        /// Gets a specific log mmuh employee by ID
        /// </summary>
        [HttpGet("{id}")]
        public async Task<IActionResult> GetLogMmuhEmployee(Guid id)
        {
            try
            {
                var log = await _context.LogMmuhEmployees.FindAsync(id);

                if (log == null)
                {
                    return NotFound(new { message = "Log mmuh employee not found" });
                }
            ...
            {
                _logger?.LogError(ex, $"Error getting log mmuh employee {id}");
                return StatusCode(500, new
                {
                    error = "Failed to get log mmuh employee",
                    message = ex.Message
                });
            }
        }
