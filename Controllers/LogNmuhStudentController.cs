...
namespace PordznakanAPI.Controllers
{
    ...
    public class LogNmuhStudentController : ControllerBase
    {
        ...
        private readonly ILogger<LogNmuhStudentController>? _logger;
        private readonly IConfiguration _configuration;
        private readonly HttpClient _httpClient;

        public LogNmuhStudentController(
            AppDbContext context,
            IConfiguration configuration,
            ILogger<LogNmuhStudentController>? logger = null)
        {
            _context = context;
        ...
        private JArray ParsePhpArrayFormat(string phpOutput)
        {
            ...
            for (int i = 0; i < objectMatches.Count; i++)
            {
                ...
                // Extract school_id - pattern: [school_id] => 22
                ...
                // Extract action_date - pattern: [action_date] => 2026-02-13 12:13:02
                ...
                // Extract method - pattern: [method] => StudentExempt
        ...
        public async Task ProcessAllRegions()
        {
            ...
            foreach (var regionId in regionIds)
            {
                try
                {
                    _logger?.LogInformation($"Processing LogNmuhStudent logs for region {regionId}...");
                    
                    // Read data from source API
                    using var sourceClient = new HttpClient();
                    var sourceUrl = $"https://api.emis.am/v1/dshh_log_nmuh_students/{regionId}";
                    ...
                    foreach (var logToken in logArray)
                    {
                        ...
                        try
                        {
                            ...
                            var existingLog = await _context.LogNmuhStudents
                            ...
                            else
                            {
                                ...
                                _context.LogNmuhStudents.Add(newLog);
                                addedCount++;
                            }
                        }
                ...
                {
                    _logger?.LogError(ex, $"Exception while processing LogNmuhStudent logs for region {regionId}");
                }
            }

            _logger?.LogInformation("LogNmuhStudent processing completed for all regions.");
        }

        /// <summary>
        /// Reads log nmuh student data from the API and saves it to the database
        /// Transfer logic is commented out for testing
        /// </summary>
        [HttpPost("transfer/{regionId}")]
        public async Task<IActionResult> TransferLogNmuhStudents([FromRoute] int regionId)
        {
            try
            {
                // Get transfer API URL from configuration (commented out - not used for now)
                // TODO: Add this to appsettings.json: "TransferApi:LogNmuhStudentUrl": "https://your-api-endpoint.com/api/log-nmuh-student"
                // var transferApiUrl = _configuration?["TransferApi:LogNmuhStudentUrl"] 
                //     ?? "https://your-api-endpoint.com/api/log-nmuh-student"; // Placeholder - replace with actual API URL
            ...
            {
                _logger?.LogError(ex, $"Error processing LogNmuhStudent for region {regionId}");
                return StatusCode(500, new
                {
        ...
        /// <summary>
        /// Reads log nmuh student data from the API and saves all regions
        /// </summary>
        ...
        public async Task<IActionResult> TransferAllRegions()
        {
            ...
            foreach (var regionId in regionIds)
            {
                try
                {
                    ...
                    var result = await TransferLogNmuhStudents(regionId);
        ...
        /// <summary>
        /// Gets all log nmuh students from database
        /// </summary>
        [HttpGet]
        public async Task<IActionResult> GetLogNmuhStudents(
            ...
            [FromQuery] int take = 100)
        {
            try
            {
                var query = _context.LogNmuhStudents.AsQueryable();
            ...
            {
                _logger?.LogError(ex, "Error getting log nmuh students");
                return StatusCode(500, new
                {
                    error = "Failed to get log nmuh students",
                    message = ex.Message
                });
            }
        }
        ...
        public async Task<IActionResult> TestDatabase()
        {
            try
            {
                var count = await _context.LogNmuhStudents.CountAsync();
                var sample = await _context.LogNmuhStudents
        ...
        /// <summary>
        /// Gets a specific log nmuh student by ID
        /// </summary>
        [HttpGet("{id}")]
        public async Task<IActionResult> GetLogNmuhStudent(Guid id)
        {
            try
            {
                var log = await _context.LogNmuhStudents.FindAsync(id);

                if (log == null)
                {
                    return NotFound(new { message = "Log nmuh student not found" });
                }
            ...
            {
                _logger?.LogError(ex, $"Error getting log nmuh student {id}");
                return StatusCode(500, new
                {
                    error = "Failed to get log nmuh student",
                    message = ex.Message
                });
            }
        }
