...
namespace PordznakanAPI.Controllers
{
    ...
    [ApiController]
    public class LogEmployeeController : ControllerBase
    {
        private readonly AppDbContext _context;
        private readonly ILogger<LogEmployeeController>? _logger;
        private readonly IConfiguration _configuration;
        private readonly HttpClient _httpClient;

        public LogEmployeeController(
            AppDbContext context,
            IConfiguration configuration,
            ILogger<LogEmployeeController>? logger = null)
        {
            _context = context;
        ...
        private JArray ParsePhpArrayFormat(string phpOutput)
        {
            ...
            for (int i = 0; i < objectMatches.Count; i++)
            {
                ...
                // Extract school_id - pattern: [school_id] => 1310
                ...
                // Extract action_date - pattern: [action_date] => 2026-02-13 13:03:15
        ...
        public async Task ProcessAllRegions()
        {
            ...
            foreach (var regionId in regionIds)
            {
                try
                {
                    _logger?.LogInformation($"Processing LogEmployee logs for region {regionId}...");
                    
                    // Read data from source API
                    using var sourceClient = new HttpClient();
                    var sourceUrl = $"https://api.emis.am/v1/dshh_log_emploee/{regionId}";
                    ...
                    foreach (var logToken in logArray)
                    {
                        ...
                        try
                        {
                            ...
                            var existingLog = await _context.LogEmployees
                            ...
                            else
                            {
                                ...
                                _context.LogEmployees.Add(newLog);
                                addedCount++;
                            }
                        }
                ...
                {
                    _logger?.LogError(ex, $"Exception while processing LogEmployee logs for region {regionId}");
                }
            }

            _logger?.LogInformation("LogEmployee processing completed for all regions.");
        }

        /// <summary>
        /// Reads log employee data from the API and saves it to the database
        /// Transfer logic is commented out for testing
        /// </summary>
        [HttpPost("transfer/{regionId}")]
        public async Task<IActionResult> TransferLogEmployees([FromRoute] int regionId)
        {
            try
            {
                // Get transfer API URL from configuration (commented out - not used for now)
                // TODO: Add this to appsettings.json: "TransferApi:LogEmployeeUrl": "https://your-api-endpoint.com/api/log-employee"
                // var transferApiUrl = _configuration?["TransferApi:LogEmployeeUrl"] 
                //     ?? "https://your-api-endpoint.com/api/log-employee"; // Placeholder - replace with actual API URL
            ...
            {
                _logger?.LogError(ex, $"Error processing LogEmployee for region {regionId}");
                return StatusCode(500, new
                {
        ...
        /// <summary>
        /// Reads log employee data from the API and saves all regions
        /// </summary>
        ...
        public async Task<IActionResult> TransferAllRegions()
        {
            ...
            foreach (var regionId in regionIds)
            {
                try
                {
                    ...
                    var result = await TransferLogEmployees(regionId);
        ...
        /// <summary>
        /// Gets all log employees from database
        /// </summary>
        [HttpGet]
        public async Task<IActionResult> GetLogEmployees(
            ...
            [FromQuery] int take = 100)
        {
            try
            {
                var query = _context.LogEmployees.AsQueryable();
            ...
            {
                _logger?.LogError(ex, "Error getting log employees");
                return StatusCode(500, new
                {
                    error = "Failed to get log employees",
                    message = ex.Message
                });
            }
        }
        ...
        public async Task<IActionResult> TestDatabase()
        {
            try
            {
                var count = await _context.LogEmployees.CountAsync();
                var sample = await _context.LogEmployees
        ...
        /// <summary>
        /// Gets a specific log employee by ID
        /// </summary>
        [HttpGet("{id}")]
        public async Task<IActionResult> GetLogEmployee(Guid id)
        {
            try
            {
                var log = await _context.LogEmployees.FindAsync(id);

                if (log == null)
                {
                    return NotFound(new { message = "Log employee not found" });
                }
            ...
            {
                _logger?.LogError(ex, $"Error getting log employee {id}");
                return StatusCode(500, new
                {
                    error = "Failed to get log employee",
                    message = ex.Message
                });
            }
        }
