...
namespace PordznakanAPI.Controllers
{
    ...
    public class LogEmployeeController : ControllerBase
    {
        ...
        private const string SourceBaseUrl = "https://api.emis.am/v1/dshh_log_emploee";

        public LogEmployeeController(ILogTransferService logTransferService)
        {
            _logTransferService = logTransferService;
        }

        private static LogEmployee MapToModel(JObject o, DateTime fallbackDate)
        {
        ...
        public Task ProcessAllRegions(DateOnly date) =>
            _logTransferService.ProcessAllRegionsAsync(SourceBaseUrl, nameof(LogEmployee), date, MapToModel);
