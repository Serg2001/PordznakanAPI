...
namespace PordznakanAPI.Controllers
{
    ...
    [ApiController]
    public class LogStudentController : ControllerBase
    {
        private readonly AppDbContext _context;
        private readonly ILogger<LogStudentController>? _logger;
        private readonly IConfiguration _configuration;
        private readonly HttpClient _httpClient;

        public LogStudentController(
            AppDbContext context,
            IConfiguration configuration,
            ILogger<LogStudentController>? logger = null)
        {
            _context = context;
        ...
        private JArray ParsePhpArrayFormat(string phpOutput)
        {
            ...
            for (int i = 0; i < objectMatches.Count; i++)
            {
                ...
                // Extract school_id - pattern: [school_id] => 322
                ...
                // Extract action_date - pattern: [action_date] => 2026-02-13 08:40:25
        ...
        public async Task ProcessAllRegions()
        {
            ...
            foreach (var regionId in regionIds)
            {
                try
                {
                    _logger?.LogInformation($"Processing LogStudent logs for region {regionId}...");
                    
                    // Read data from source API
                    using var sourceClient = new HttpClient();
                    var sourceUrl = $"https://api.emis.am/v1/dshh_log_students/{regionId}";
                    ...
                    foreach (var logToken in logArray)
                    {
                        ...
                        try
                        {
                            ...
                            var existingLog = await _context.LogStudents
                            ...
                            else
                            {
                                ...
                                _context.LogStudents.Add(newLog);
                                addedCount++;
                            }
                        }
                ...
                {
                    _logger?.LogError(ex, $"Exception while processing LogStudent logs for region {regionId}");
                }
            }

            _logger?.LogInformation("LogStudent processing completed for all regions.");
        }

        /// <summary>
        /// Reads log student data from the API and saves it to the database
        /// Transfer logic is commented out for testing
        /// </summary>
        [HttpPost("transfer/{regionId}")]
        public async Task<IActionResult> TransferLogStudents([FromRoute] int regionId)
        {
            try
            {
                // Get transfer API URL from configuration (commented out - not used for now)
                // TODO: Add this to appsettings.json: "TransferApi:LogStudentUrl": "https://your-api-endpoint.com/api/log-student"
                // var transferApiUrl = _configuration?["TransferApi:LogStudentUrl"] 
                //     ?? "https://your-api-endpoint.com/api/log-student"; // Placeholder - replace with actual API URL
            ...
            {
                _logger?.LogError(ex, $"Error processing LogStudent for region {regionId}");
                return StatusCode(500, new
                {
        ...
        /// <summary>
        /// Reads log student data from the API and saves all regions
        /// </summary>
        ...
        public async Task<IActionResult> TransferAllRegions()
        {
            ...
            foreach (var regionId in regionIds)
            {
                try
                {
                    ...
                    var result = await TransferLogStudents(regionId);
        ...
        /// <summary>
        /// Gets all log students from database
        /// </summary>
        [HttpGet]
        public async Task<IActionResult> GetLogStudents(
            ...
            [FromQuery] int take = 100)
        {
            try
            {
                var query = _context.LogStudents.AsQueryable();
            ...
            {
                _logger?.LogError(ex, "Error getting log students");
                return StatusCode(500, new
                {
                    error = "Failed to get log students",
                    message = ex.Message
                });
            }
        }
        ...
        public async Task<IActionResult> TestDatabase()
        {
            try
            {
                var count = await _context.LogStudents.CountAsync();
                var sample = await _context.LogStudents
        ...
        /// <summary>
        /// Gets a specific log student by ID
        /// </summary>
        [HttpGet("{id}")]
        public async Task<IActionResult> GetLogStudent(Guid id)
        {
            try
            {
                var log = await _context.LogStudents.FindAsync(id);

                if (log == null)
                {
                    return NotFound(new { message = "Log student not found" });
                }
            ...
            {
                _logger?.LogError(ex, $"Error getting log student {id}");
                return StatusCode(500, new
                {
                    error = "Failed to get log student",
                    message = ex.Message
                });
            }
        }
